...
#define EAP_ANONYMOUS_IDENTITY "anonymous@cvut.cz" //anonymous@example.com, or you can use also nickname@example.com
#define EAP_IDENTITY "silpomar@cvut.cz" //nickname@example.com, at some organizations should work nickname only without realm, but it is not recommended
#define EAP_PASSWORD "xxx" //password for eduroam account
#define EAP_USERNAME "silpomar@cvut.cz" // the Username is the same as the Identity in most eduroam networks.
...
void setup() {
  ...
  const char* host = "seznam.cz";
    IPAddress ip;
    
    if (WiFi.hostByName(host, ip)) {
        Serial.print("IP adresa pro ");
        Serial.print(host);
        Serial.print(" je: ");
        Serial.println(ip);
    } else {
        Serial.println("Překlad se nezdařil");
    }
}
